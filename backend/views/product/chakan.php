...
$this->title = '发布详情';
// var_dump($desc->certificationdata);
    ...
    <div class="col-lg-10">
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped">
                ...
                <?php }else{ ?>
                    <?php $judicial = unserialize($desc['judicialstatusA'])?>
                    <?php $credi = unserialize($desc['creditorfile'])?>
                    <?php $creditorinfo = unserialize($desc['creditorinfo'])?>
                    <?php $borrowinginfo = unserialize($desc['borrowinginfo'])?>
                    <tr>
                        <td>ID</td>
                        <td><?php echo $desc['id']?$desc['id']:''?></td>
                    </tr>
                    <tr>
                        <td>类型</td>
                        <td><?php if($desc['category']==2){echo '催收';}else{echo '诉讼';}?></td>
                    </tr>
                    <tr>
                        <td> 借款本金<b>(万元)</b></td>
                    ...
                    <tr>
                        <td>  抵押<b>(可多选)</b></td>
                        <td>
			<?php if(is_numeric($desc['guaranteemethod'])){ ?>
                        <?php }else{?>
                        <?php $value = unserialize($desc['guaranteemethod']);$guar_other = $value['other'];unset($value['other']); ?>
                         <?php } ?>
                            <?php if(is_numeric($desc['guaranteemethod'])){ ?>
                                <?php echo $desc['guaranteemethod']==0?'无':'有' ?>
                            <?php }else{ ?>
                    ...
                    <tr>
                        ...
                        <td> <?php echo isset($desc['agencycommission'])?\common\models\creditorProduct::$agencycommissiontype[$desc['agencycommissiontype']]:'暂无';?>
                            <?php if($desc['agencycommissiontype']==1){?>
                                <span><?php echo $desc['agencycommission'] ?></span><?php echo isset($desc["agencycommission"])?'万':'';?>
                            <?php }else{ ?>
				...
				<?php } ?>
                <?php } ?>
               <?php  $disposingprocess = \common\models\DisposingProcess::find()->Where(['category'=>$desc['category'],'product_id'=>$desc['id']])->orderBy('create_time desc')->all();
                      $delay = \common\models\DelayApply::findOne(['category'=>$desc['category'],'product_id'=>$desc['id'],'is_agree'=>1]);
                      $apply = \common\models\Apply::findOne(['category'=>$desc['category'],'product_id'=>$desc['id'],'app_id'=>1]);
               ...
               ?>
                <?php if($disposingprocess){?>
                    ...
                    <tr>
                        <td>类型</td>
                        <td><?php $arr = ['1'=>'融资','2'=>'催收','3'=>'诉讼',];echo $arr[$desc['category']];?></td>
                    </tr>
                    <tr>
                        <td>申请日期</td>
...
<?php 
if($desc->certification){
	echo "<h3>用户认证信息</h3>";
	echo $this->render("/certification/chakan",['certi'=>$desc->certification]);
	$this->title = '发布详情';
}
?>