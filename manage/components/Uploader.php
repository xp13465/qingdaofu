 ...
 * 
 */
// defined('TEMP_PATH') or define('TEMP_PATH','uploads/images/');
define('TEMP_PATH','uploads/images/');
...
class Uploader extends Object{    
    ...
    public function uploadSimple($name = '')
    {
        if(isset($this->config[$this->model])) {
            ...
            //获取一个上传实例
            $ins = UploadedFile::getInstanceByName($name);
            if(!$ins) {
    ...
    public function initResumable($model = '')
    {
        ...
        $this->model = $model;
        //文件ID
        $this->file_id      = Yii::$app->request->get('resumableIdentifier');
        //原始文件名               
        $this->real_name    = Yii::$app->request->get('resumableFilename');
        //文件总大小               
        $this->total_size   = Yii::$app->request->get('resumableTotalSize');
        //文件扩展名
		//var_dump($this->real_name);die;
        $this->file_ext     = Helper::getExtensionName($this->real_name);   
        		
        //当前片段编号
        $this->_chunk_num   =  Yii::$app->request->get('resumableChunkNumber');
        //每个片段大小
        $this->_chunk_size  =  Yii::$app->request->get('resumableChunkSize');
        return $this;
    }
    ...
    public function uploadResumable($name = '')
    {
        ...
        if(isset($this->config[$this->model])) {
            //获取一个上传实例 getInstanceByName
		    //$ins = \frontend\components\UploadedFile::getInstance($model,$name);
    ...
    {
        $file_id      = Yii::$app->request->get('resumableIdentifier');
        $file_name    = Yii::$app->request->get('resumableFilename');
        $chunk_number = Yii::$app->request->get('resumableChunkNumber'); 
        $temp_dir     = $this->_getChunkDir($file_id, false);
        $convert_name = $this->_convertChineseName($file_name);
        $chunk_file   =  str_replace(array('\\', '\\\\'), '/', $temp_dir . $convert_name.'.part'.$chunk_number); 
    ...
    {
        return $name;//Helper::getOS() == 'Windows' ? Helper::safeEncoding($name, 'GB2312') : $name;
    }
