...
class ProductsController extends BackController
{
    ...
    public function actionView($id='')
    {
		$query = Product::find();
		$data = $query->view($id);
		$progress = $query->ProductData($data);
		$model = $this->findModel($id,$Handle=true);
		$logGroup = $model['productOrders']['productOrdersLogs']?\yii\helpers\ArrayHelper::index($model['productOrders']['productOrdersLogs'],null,'action'):[];
		
	    foreach($logGroup as $action=>$CurNodeData){
			$CurNodeData['name']="";
			$CurNodeData['time']="";
			$CurNodeData['logNum']= count($CurNodeData);
			$CurNodeData['fileNum']=0;
			foreach($CurNodeData as $k=>$va){
				if(!is_numeric($k))continue;
				if(isset($va['actionUser'])){
					$name = $va['actionUser']['realname']?$va['actionUser']['realname']:$va['actionUser']['username'];
					$CurNodeData['name'] .= $CurNodeData['name']?(",".$name):$name;
				}
				$CurNodeData['time'] =  date('Y-m-d',$va['action_at']);
				$CurNodeData['timeint'] =  $va['action_at'];
				$fileNum = $va['files']?count(explode(",",$va['files'])):0;
				$CurNodeData['fileNum'] = $CurNodeData['fileNum']+$fileNum;
			}
			$logGroup[$action]=$CurNodeData;
		}
	    // echo "<pre>";
		// var_dump($model);die;
        return $this->render('view', [
            'model' =>$model, //$this->findModel($id,$Handle=true),
            'data' =>$data, //$this->findModel($id,$Handle=true),
			'progress'=>$progress,
			'logGroup'=>$logGroup,
        ]);
    }
	...
	*/
	...
	public function actionDownzip($id){
		$model = $this->findModel($id,true);
		$actionLabels = \app\models\ProductOrdersLog::$actionLabels;
		$logGroup = $model['productOrders']['productOrdersLogs']?\yii\helpers\ArrayHelper::index($model['productOrders']['productOrdersLogs'],null,'action'):[];
		
		$datalist =[];
		$frontPath ="../../frontend/web";
// var_dump($model["pacts"]);exit;
		if(isset($model["pacts"])&&$model["pacts"]){
			foreach($model["pacts"] as $pact){
				$datalist["服务合同". DIRECTORY_SEPARATOR .$pact['name']] = $frontPath. DIRECTORY_SEPARATOR .$pact['addr'];
			}
		}
		foreach($logGroup as $action=>$log):
			$StatusLabel = isset($actionLabels[$action])?$actionLabels[$action]:$action;
			foreach($log as $l):
				foreach($l['filesImg'] as $file):
					$datalist[$StatusLabel. DIRECTORY_SEPARATOR .$file['name']] = $frontPath. DIRECTORY_SEPARATOR .$file['addr'];
				endforeach;
			endforeach;
		endforeach;
		// echo 4;
		// var_dump($datalist);
		$zipName = "./download/".$model['number']."_files.zip"; 
		$status = \manage\services\Func::file2Zip($datalist,$zipName,true,$errorMsg);
		echo $errorMsg;
	}
}
