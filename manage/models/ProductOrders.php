...
class ProductOrders extends \yii\db\ActiveRecord
{
	...
	public function getOrdersLogs($orderBy='logs.action_at desc,logs.logid desc'){
		return $this->hasMany(ProductOrdersLog::className(), ['ordersid' => 'ordersid'])->alias('logs')->onCondition(['logs.validflag'=>1])->orderBy($orderBy)->joinWith(['user']);
	}
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getProductOrdersOperators($isArray = true,$orderBy='(case operators.level when 1 then operators.operatorid else operators.create_by end) asc ,operators.level asc,operators.create_at asc ')
    {
		if($isArray){
    ...
    {
        return $this->hasMany(ProductOrdersTermination::className(), ['ordersid' => 'ordersid'])->alias('terminations')->onCondition(['terminations.validflag'=>'1']);
    }
	...
	*/
	public function create($data=[] , $create_by = ''){
		...
		if($this->save()){
			...
			$ProductOrdersLog->create($this->ordersid,'0','面谈成功，自动生成订单',$beforeStatus,$afterStatus,10,1,$create_by);
			return OK;
		}else{
	...
	*	
	*/
	public function closed($modify_by = '',$params=[]){
		$modify_by = $modify_by?$modify_by:Yii::$app->user->getId();
		$status = $this->updateAll(['modify_by'=>$modify_by,'modify_at'=>time(),'status'=>'40'],["ordersid"=>$this->ordersid,'status'=>'20']);
		if($status){
			if($params){
				$memo = '订单结案，回款金额：'.$params['backmoney'].'元,支付利息:'.$params['payinterest'].'元,佣金:'.$params['actualamount'].'元.';
			}else{
				$memo = '订单结案.';
			}
			$ProductOrdersLog = new ProductOrdersLog;
			$ProductOrdersLog->create($this->ordersid,"0",$memo,20,40,28,1,$modify_by);
			return OK;
		}else{
			return MODELDATASAVE;
		}
	...
	*/
    ...
    {
		if($this->pact){
			return \app\models\QdfFiles::getFiles(explode(",",$this->pact),['id','name','file','addr']);
		}else{
			return [];
		}
    }
