...
class ProductOrdersQuery extends \yii\db\ActiveQuery
{
	...
	*/
	public function ordersGenerate($applyid = null, $uid = ''){
		...
		// var_dump($applyid);
		// var_dump($apply->productOrders);
		if($apply->productOrders)return 'EXISTED';
		$status = $ProductOrders->create($apply->toArray(),$uid);
	...
	* @return status 结案结果
	*/
	public function ordersClosedAgree($closedid = null, $resultmemo='', $uid = '',$params=[]){
		...
		$status = $Closed->orders->closed($uid,$params);
		if($status!='ok'){
			$innerTransaction->rollBack();  
	...
	public function commentList($ordersid,$isobj = true){
		...
		foreach($productOrdersOperators ["Comments1"] as $key => $value){
			  if($value['files']){
				 ...
				 $productOrdersOperators ["Comments1"][$key]["filesImg"]=\app\models\QdfFiles::getFiles($ids); 
			  }
		}
		foreach($productOrdersOperators ["Comments2"] as $key => $value){
			  if($value['files']){
				 $ids=explode(",",$value["files"]);
				 $productOrdersOperators ["Comments2"][$key]["filesImg"]=\app\models\QdfFiles::getFiles($ids); 
			  }
		}
				...
				if($data['orders']&&$data['orders']['productOrdersLogs']){
					...
					/*foreach($data['orders']['productOrdersLogs'] as $key => $productOrdersLogs){
						...
						if($productOrdersLogs["files"]){
							...
							$data['orders']['productOrdersLogs'][$key]["filesImg"]=\app\models\QdfFiles::getFiles($ids);
						}else{
