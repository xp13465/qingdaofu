...
class ProductOrdersLog extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasOne(User::className(), ['id' => 'action_by'])
		->alias("actionuser")
    ...
    {
        return $this->hasMany(QdfFiles::className(), ["id"=>"files"])->alias("fileImg");
    }
	
	public function getUser(){
		return $this->hasOne(User::className(),['id' => 'action_by'])->alias('user')->select('user.id,user.username,user.mobile');
	}
	...
	public static function filterAll($Logs,$accessTerminationAUTH,$accessClosedAUTH,$checkStatus,$fabuid,$jiedanid,$operatorsData=[],&$data=[]){
		...
		foreach($Logs as $key => $productOrdersLogs){
			...
			if($productOrdersLogs["files"]){
				...
				$Logs[$key]["filesImg"]=\app\models\QdfFiles::getFiles($ids);
			}else{
				$Logs[$key]["filesImg"]=[];
			}
			...
			if($productOrdersLogs['action']!=10){
				...
				if($productOrdersLogs['action_by']==$userid){
					...
					if(isset($Operators[$productOrdersLogs['action_by']])){
						$operator = User::findUserData($Operators[$productOrdersLogs['action_by']]["operatorid"],['mobile','realname']);
						$label = "经";
						$class = "jing";
	...
	public function filterAlls($data=[],$fabuid,$jiedanid,$operatorsData){
		...
		foreach($data as $key=>$value){
			...
			if($value["files"]){
				...
				$data[$key]["filesImg"]=\app\models\QdfFiles::getFiles($ids);
			}else{
				$data[$key]["filesImg"]=[];
			}
			...
			if($value['action']!=10){
				...
				if($value['action_by']==$userid){
					...
					if(isset($Operators[$value['action_by']])){
						$operator = User::findUserData($Operators[$value['action_by']]["operatorid"],['mobile','realname']);
						$label = "经";
						$class = "jing";
