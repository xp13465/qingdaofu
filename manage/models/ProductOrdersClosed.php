...
class ProductOrdersClosed extends \yii\db\ActiveRecord
{
    ...
    {
        return 'zcb_product_orders_closed';
    }
	...
	*/
	public function apply($params=[] , $create_by = ''){
		...
		if($this->save()){
			...
			$ProductOrdersLog->create($this->ordersid,$this->closedid,$this->applymemo,$beforeStatus,$afterStatus,40,2,$this->create_by);
			return OK;
		}else{
	...
	public function agree($resultmemo='',$modify_by = ''){
		...
		if($status){
			...
			$ProductOrdersLog->create($this->ordersid,$this->closedid,$resultmemo,20,40,41,2,$modify_by);
