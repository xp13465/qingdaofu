...
class Func{
    ...
    public static function createCatCode($cat = 1){
        ...
        if($cat == 1){
			...
			$code = self::getNextCode($dbArr[count($dbArr)-1].'.zcb_product','BX','number','create_at');
        }
        return $code;
    }
	 ...
	 *
	 */
	public static function file2Zip($datalist,$zipName,$isDown = false,&$errorMsg=""){
		// $zipName = "./download/case20170223dd.zip"; //最终生成的文件名（含路径）   
		if(file_exists($zipName))unlink($zipName);
		if(!file_exists($zipName)){   
			//重新生成文件   
			$zip = new \ZipArchive();//使用本类，linux需开启zlib，windows需取消php_zip.dll前的注释   
			if ($zip->open($zipName, \ZIPARCHIVE::CREATE)!==TRUE) {   
				$errorMsg = '无法打开文件，或者文件创建失败';
				return false;
			}   
			foreach( $datalist as $key=>$val){   
				if(file_exists($val)){   
					$source = $val;
					$outfile = is_numeric($key)?basename($val):$key;
					$zip->addFile( $source, $outfile);//第二个参数是放在压缩包中的文件名称，如果文件可能会有重复，就需要注意一下   
				}   
			}   
			$zip->close();//关闭   
		}
