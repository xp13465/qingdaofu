...
<div class="content_right">
    <div class="c_right_bottom security clearfix">
        <div class="warning">
            <span>安全级别</span><i></i><i class="zise"></i>　　　　　　<span>较高</span>　　　
        </div>
        <div class="setmima">
            <div class="mima">
                <i class="display"></i>
                <span class="display">登录密码</span>
                <p class="display">由于互联网存在被盗号的风险,清道夫债管家建议您设置安全级别更高的密码,并建议您定时修改,确保您的账号安全.</p>
                <input type="button" value="修改" class="password">
                <form class="pass">
                <div class="modify" style="<?=$type?"":"display:none"?>">
                    <em class="display"></em>
                    ...
                    <div>
						<?= Html::tag('input',"1",['type'=>'button','value'=>($type==2?"保存":'修改'),'id'=>'currents','class'=>'denglu change'])?>
                    </div>
                </div>
                </form>
            </div>
            <!--<div class="yanz">
                ...
                <div class="replace">
                    ...
                    <div>
                        ...
                        <input type="text" name="mobile" readonly id="" value="<?=Yii::$app->user->identity->mobile?>">
        ...
        <div class="tishi">
            <span>安全服务提示</span>
            <p>
                1.确认您登录的是清道夫债管家网址：http//www.zcb2016.com,注意防范进入钓鱼网站,不要轻信各种工具发送的信息,谨防诈骗.
            </p>
            <p>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#currents').click(function () {
            ...
            var url = "<?php echo Url::toRoute(($type==2?"/userinfo/setpassword":'/userinfo/modifypassword'))?>";
			var index = layer.load(1);
            $.post(url, {old_password:current,new_password:passwords}, function (json) {
				layer.close(index)
                if(json.code == '0000'){
                    layer.msg(json.msg,{},function(){ location.href = "<?php echo Url::toRoute('/user/security')?>";});
                }else{
                    layer.msg(json.msg);
                }
            }, 'json');
    ...
    $(function(){
        ...
        $('.phone').click(function(){
           ...
           var verifyCode = $('input[name=verifyCode]').val();
            ...
            var url = "<?php echo Yii\helpers\Url::toRoute('/user/modifymobile')?>";
            $.post(url,$('.modifyphone').serialize(),function(i){
                      if(i == 1){
                          ...
                          alert('修改成功');
                          location.href = "<?php echo yii\helpers\Url::toRoute('/user/security')?>";
                      }
            },'json');
        })
    })*/
