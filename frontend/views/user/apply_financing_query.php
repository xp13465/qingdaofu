...
<div class="details">
    ...
    <div class="finance">
        ...
        <!-- <i></i> -->
        <?php echo $this->renderFile('@app/views/public/pubdetail.php',['category'=>1,'id'=>$id])?>
    ...
    $(function(){
        ...
        $("#apply_submit").off().on("click", function(){
            if($("input[name='idlist[]']:checked").length > 0){
                ...
                var url = "<?php echo Url::toRoute('/user/determine_financing')?>";
                $.post(url,{idlist:idlist},function(v){
                     if(v == 0){
                         alert('请不要重复确认');
                     }else if(v == 1){
                         alert('已确认');
                         location.href="<?php echo Url::toRoute('/inquire/termination')?>";
                     }else{
                         alert('提交失败');
                     }
                })
