...
?>
    <table cellspacing="1" cellpadding="0">
        ...
        <tr>
            ...
            <td>
                <?php echo Yii::$app->db->createCommand("select count(*) from zcb_apply where category = {$v['category']} and product_id = {$v['id']} and app_id = 0 ")->queryScalar(); ?>
            </td>
...
<div class="fenye clearfix"><?= linkPager::widget([
    'firstPageLabel' => '首页',
    'lastPageLabel' => '最后一页',
    'prevPageLabel' => '上一页',
    'nextPageLabel' => '下一页',
...
<script type="text/javascript">
    function inquire(id,uid) {
        var user_url = '';
        if(uid == '1'){
            user_url = "<?php echo Url::toRoute('/apply/financing')?>";
        } else if(uid == '2'|| uid == '3'){
            user_url = "<?php echo Url::toRoute('/apply/creditor')?>";
        } else {
            user_url = FALSE;
        }
        if(user_url) {
