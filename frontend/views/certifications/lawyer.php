...
<div class="content_right1">
		...
		?>
          <div class="detail pl" style="display: block;">
		   <?= Html::activeHiddenInput($model,'category',array('value'=>2)) ?>
            <ul>
              <li> <span><i class="red">*</i>律所名称 </span> 
					<?= $form->field($model,'name',['options'=>['tag'=>'code'],'template'=>'{input}{error}'])->textInput(['placeholder'=>'真实律所名称，需与证件一致','style'=>"width: 300px;"]);?>
			  </li>
              <li> 
					<span><i class="red">*</i>执业证号</span>
					<?= $form->field($model,'cardno',['options'=>['tag'=>'code'],'template'=>'{input}{error}'])->textInput(['placeholder'=>'填写执业证号','style'=>'width:300px']);?>
			  </li>
              <li class='pic'>
          ...
          <input class="btn buttonev01" type="button" value="立即认证" onclick="$('#lawyer').submit()" >
        <?php ActiveForm::end();?>
</div>
...
$(document).ready(function(){
	...
	$('#lawyer').on('beforeSubmit',function(e){
		if(!$("#cardimg1").val()){
			return false;
		}
		$.ajax({
