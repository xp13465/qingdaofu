...
<div class="details">
    <div class="look">
        <h3>查看信息</h3><a href="<?php echo yii\helpers\Url::toRoute('/order/index')?>"><img src="/images/close1.png" style="float: right;margin: -25px 10px 0px 0px;"></a>
    </div>
    <!-- 融资详情 -->
    <div class="finance">
    ...
    if(in_array($dt->progress_status,[1])){
        echo $this->renderFile('@app/views/public/apply.php',['category'=>$dt['category'],'id'=>$dt['id']]);
    }
    ?>
    <?php
    //协议
    if(in_array($dt->progress_status,[2,3,4])){
    ...
    if(in_array($dt->progress_status,[2])){
        echo $this->renderFile('@app/views/public/disposingprocessadd.php', ['category' => $dt['category'], 'id' => $dt['id']]);
    }
    ?>
    <?php
    //处置进度
    ...
    //延迟申请
    if(in_array($dt->progress_status,[2])){
        echo $this->renderFile('@app/views/public/delay.php', ['category' => $dt['category'], 'id' => $dt['id']]);
    }
    ?>
    ...
    if(in_array($dt->progress_status,[2]) && isset($status) && in_array($status,[3,4])){
    ...
    <?php
    //评价
    if(in_array($dt->progress_status,[3,4])){
