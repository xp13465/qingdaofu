<?php
namespace frontend\extensions\sms;
...
class Smser
{
    ...
    public function sendValidateCode($mobile,$type = 1){
        ...
        if($type == 2){
            $msgText = '修改验证码：'.$validateCode.'，在10分钟内有效。修改成功后，可以用新密码登录。请勿向他人泄露验证码。【直向资产】';
        }else if($type == 3){
            $msgText = '验证码：'.$validateCode.'，有效时间10分钟，为了保护您的帐号安全，验证短信请勿转发给其他人。【直向资产】';
        }else if($type == 4){
            $msgText = '修改绑定手机验证码：'.$validateCode.'，有效时间10分钟，为了保护您的帐号安全，验证短信请勿转发给其他人。【直向资产】';
        }else{
            $msgText = '验证码：'.$validateCode.'，有效时间10分钟，为了保护您的帐号安全，验证短信请勿转发给其他人。【直向资产】';
        }
        $postData = [
