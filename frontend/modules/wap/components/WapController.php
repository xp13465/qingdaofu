...
class WapController extends Controller
{
    ...
    public function init(){
		...
		if(!YII_ENV_TEST&&!Yii::$app->request->isPost){
			...
			if($requestedRoute{0}=="/")$requestedRoute=substr($requestedRoute,1);
			// echo $requestedRoute;
			$continueArr =[
				"wap/product/index",
				"wap/product/detail",
				"wap/product/demo",
				"wap/productorders/demo",
			];
			if(in_array($requestedRoute,$continueArr))$type = false;
			$this->loadUser($type);
        }
    }
	...
	*/
	protected function success($msg='',$data=array(),$type='json'){
		...
		if(!$debug){
			echo ($Msg);
			// echo $this->render("/layouts/empty");
			// exit;
		}else{
			echo($Msg);
			echo $this->render("/layouts/debug");
			exit;
		}
	...
	*/
	...
	protected function loadUser($doExit = true){
		...
		// if(!$token&&YII_ENV_TEST){
			// $token = User::findOne(['id'=>Yii::$app->params['testUid']])['token'];
		// }
		if($token){
