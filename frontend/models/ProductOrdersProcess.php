...
class ProductOrdersProcess extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['type', 'ordersid',  'productid'], 'required'],
			
			['memo','required', 'when' => function ($model) {
				return  $model->type === '0';
			},'message'=>'留言不能为空'],
			['memo','required', 'when' => function ($model) {
				return   $model->type !== '0' && empty($model->files);
			},'message'=>'图片和文本必须填写一样'],
			
			
			
            [['type', 'validflag'], 'string'],
			['type','in','range'=>['0','1','2','3','4','5','6','7','8'],'message'=>'请选择正确的进度类型'],
