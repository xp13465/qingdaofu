...
class ProductOrdersQuery extends \yii\db\ActiveQuery
{
	...
	*/
	public function ordersProcessAdd($params,$type=1, $uid = ''){
		...
		$actions = \app\models\ProductOrdersProcess::$actions;
		...
		if($status == 'ok'){
			$actionLabel = isset($actions[$ProductOrdersProcess->type])?$actions[$ProductOrdersProcess->type]:"";
			$message = Message::find();
			if($ProductOrdersProcess->create_by == $ProductOrdersProcess->product->create_by){
				$data = $message->addMessage(117,['code'=>$ProductOrdersProcess->product->number,'action'=>$actionLabel],$ProductOrdersProcess->orders->create_by,$ProductOrdersProcess->orders->applyid,50,$ProductOrdersProcess->product->create_by);
			}else{
				$data = $message->addMessage(201,['code'=>$ProductOrdersProcess->product->number,'action'=>$actionLabel],$ProductOrdersProcess->product->create_by,$ProductOrdersProcess->productid,40,$ProductOrdersProcess->create_by);
			}
			foreach($ProductOrdersProcess->orders->productOrdersOperators as $ordersOperators){
				if($ordersOperators->operatorid==$ProductOrdersProcess->create_by){
					$message->addMessage(211,['code'=>$ProductOrdersProcess->product->number,'action'=>$actionLabel],$ProductOrdersProcess->orders->create_by,$ProductOrdersProcess->orders->applyid,50,$ProductOrdersProcess->create_by);
				}else{
					$messageTemp = $ProductOrdersProcess->create_by == $ProductOrdersProcess->product->create_by?117:201;
					$message->addMessage($messageTemp,['code'=>$ProductOrdersProcess->product->number,'action'=>$actionLabel],$ordersOperators->operatorid,$ProductOrdersProcess->orders->applyid,50,$ProductOrdersProcess->create_by);
				}
			}
			return $status;
		}else{
	...
	*/
	public function ordersClosedDetail($closedid = null){
		...
		if(!$Closed||!$Closed->orders->accessClosed('READ')){
			return false;
		}
		return $Closed;
	}
	...
	*/
	public function ordersTerminationDetail($terminationid = null){
		...
		if(!$Termination||!$Termination->orders->accessTermination('READ')){
			return false;
		}
		return $Termination;
	}
	...
	* @return options
	*/
	public function ordersProcessActions($orderid=""){
		$hasAction = [];
		if($orderid){
			$data = \app\models\ProductOrdersProcess::find()->where(["validflag"=>"1","ordersid"=>$orderid])->select(["type"])->asArray()->groupBy("type")->all();
			foreach($data as $val){
				$hasAction[]=$val["type"];
			}
		}
		$data = \app\models\ProductOrdersProcess::$actions;
		$actions = [];
		foreach($data as $key=>$val){
			$disabled =1;
			if(in_array($key,[0,1,2])){
				$disabled =0;
			}else if(in_array($key,[3])&&in_array(2,$hasAction)){
				$disabled =0;
			}else if(in_array($key,[4,5,6])&&in_array(3,$hasAction)){
				$disabled =0;
			}else if(in_array($key,[7])&&in_array(4,$hasAction)&&in_array(5,$hasAction)&&in_array(6,$hasAction)){
				$disabled =0;
			}else if(in_array($key,[8])&&in_array(7,$hasAction)){
				$disabled =0;
			}
			$actions[]=["id"=>$key,"name"=>$val,"disabled"=>$disabled];
		}
		return $actions;
	}
