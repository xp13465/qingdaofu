	...
	public function load($data, $formName = NULL){
		if (parent::load($data, $formName)) {
			...
			if ($scope === '' && !empty($data)) {
				...
				$icon = isset($data[$scope]["icon"])?$data[$scope]["icon"]:'';
			}else{
				$icon = "";
			}
			$isondata=$this->data?\yii\helpers\Json::decode($this->data):["icon"=>"fa","visible"=>true];
			if($icon){
				$isondata["icon"]="fa ".$icon;
			}
			$this->data = \yii\helpers\Json::encode($isondata);
