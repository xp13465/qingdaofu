...
<div class="details">
    ...
    <div class="affirm">
        <a href="#" onclick="applys('<?php echo $dt['id']?>','<?php echo $dt['category']?>')">申请</a>
        <br/>
    </div>
</div>
<script type="text/javascript">
    function applys(id,cate){
        var save_url = '';
        if(cate == 1){
             save_url = "<?php echo Url::to('/order/saveapp')?>";
        }else if(cate == 2 || cate == 3){
            save_url = "<?php echo Url::to('/order/savecoll')?>";
        }else{
            save_url = FALSE;
        }
        $.post(save_url,{id:id,cate:cate},function(v){
            if(v == 1){
                ...
                alert('已发出申请');
                location.href = "<?php echo Url::to('/order/ordertermination')?>";
            }else{
                alert('申请失败');
            }
        },'json');
