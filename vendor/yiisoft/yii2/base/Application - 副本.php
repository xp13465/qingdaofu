...
abstract class Application extends Module
{
    ...
    {
        try {
			$MSG= '开始内存：'.$c1=memory_get_usage(). '<br>'; 
		...
		$MSG.= '运行后内存：'.$c2=memory_get_usage().'<br>';  
	 
		if (($mem_usage=$c2-$c1) < 1024) 
            $MSG.= $mem_usage." bytes"; 
        elseif ($mem_usage < 1048576) 
            $MSG.= round($mem_usage/1024,2)." kilobytes"; 
        else 
            $MSG.= round($mem_usage/1048576,2)." megabytes"; 
