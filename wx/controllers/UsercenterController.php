...
class UsercenterController extends \wx\components\FrontController{
    ...
    public function actionRelease(){
        ...
        $str    = Func::CurlPost(yii\helpers\Url::toRoute('/wap/apply/released'),['token' => $token ,'progress_status' => $status,'limit'=>10,'page'=>$page]);
		$arr    = $this->json2array($str);
        if($arr['code'] == '0000'){
    ...
    public function actionUserinfo(){
        ...
        if($arr['code'] == '0000'){
            ...
            $uid           = $arr['result']['uid'];
            $pid           = $arr['result']['pid'];
            return $this->render('userinfo',['certi'=>$certification,'uid'=>$uid,'product'=>isset($product)?$product:'','pid'=>$pid]);die;
        }else{
    ...
    public function actionOrdersuser(){
        ...
        if($arr['code'] == '0000'){
          ...
          //  echo Func::CurlPost(yii\helpers\Url::toRoute('/wap/apply/user'),['token'=>$token,'id'=>$id,'category'=>$category]);
            $product       = $arr['product'];
            return $this->render('ordersuser',['product'=>$product]);die;
        }
    ...
    public function actionOrders(){
        ...
        $str = Func::CurlPost(yii\helpers\Url::toRoute('/wap/apply/index'),['token'=>$token,'page'=>$page,'progress_status'=>$pro,'status'=>$status,'limit'=>10]);
		$arr = $this->json2array($str);
        if($arr['code'] == '0000'){
            $rows       = $arr['result']['rows'];
            $page       = $arr['result']['page'];
            $creditor   = $arr['result']['creditor'];
            $delays     = $arr['result']['delays'];
            return $this->render('orders',['rows'=>$rows,'creditor'=>$creditor,'delays'=>$delays]);
        }else{
    ...
    public function actionPreservation(){
		// $this->layout="newmain";
        $this->title = "清道夫债管家-我的草稿";
        $this->keywords = "清道夫债管家-我的草稿";
        $this->description = "清道夫债管家-我的草稿";
        $token = Yii::$app->session->get('user_token');
        $page = Yii::$app->request->get('page');
        $str = Func::CurlPost(yii\helpers\Url::toRoute('/wap/product/preservation'),['token'=>$token,'limit'=>10,'page'=>$page]);
		$arr = yii\helpers\Json::decode($str);
        if($arr['code'] == '0000'){
            $Preserv = $arr['result']['data'];
			return $this->render('preservation',['data'=>$Preserv]);
        }else{
			$this->notFound();
		}
    ...
    public function actionCollection(){
        ...
        $token = Yii::$app->session->get('user_token');
		$page = Yii::$app->request->get('page');
        $str = Func::CurlPost(yii\helpers\Url::toRoute('/wap/product/collect-list'),['token'=>$token,'limit'=>10,'page'=>$page]);
        $arr = yii\helpers\Json::decode($str);
        if($arr['code'] == '0000'){
            $data = $arr['result']['data'];
			return $this->render('collection',['data'=>$data]);
        }else{
			$this->notFound();
		}
