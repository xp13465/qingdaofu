...
<section>
    ...
    <ul class="infor">
        <li>
            <div class="infor_l">
                <span>金额</span>
                <?php echo Html::input('text','money','',['placeholder'=>'填写您希望融资的金额'])?>
                <span>万元</span>
            </div>
        </li>
        <li>
...
<footer>
    <ul class="hold">
        <li class="saveCls">
            ...
            <?php echo Html::hiddenInput('id',Yii::$app->request->get('id'))?>
...
<script>
    $(function(){
        ...
        function setDefaultValue(){
            if(getCookie('financing') == null || getCookie('financing') == '' ){
                return false;
            }
            var all = formUnserialize(getCookie('financing'));
        ...
        function save(type){
            $.ajax({
                url:"<?php echo \yii\helpers\Url::toRoute(['/publish/editfinancing','id'=>Yii::$app->request->get('id'),'category'=>Yii::$app->request->get('category')])?>",
                type:'post',
    ...
    function saveCookie(){
        var sc = setCookie(getCookie('publishCookieName'),$('form').serialize(),'s100');
    }
