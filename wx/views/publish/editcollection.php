...
?>
<?=wxHeaderWidget::widget(['title'=>'发布清收','gohtml'=>'<a href="javascript:void(0)" class="publishCls">保存</a>'])?>
	...
	<ul class="infor">
        ...
        <div class="infor_l">
         <span>费用类型</span>
		  ...
		  <?php echo Html::dropDownList('agencycommissiontype',isset($datas['agencycommissiontype'])?$datas['agencycommissiontype']:'',[''=>'请选择','1'=>'服务佣金','2'=>'固定费用'],['style'=>"width:65%;left:35%;height:50px;"])?>
          </div>
          </span> 
        </div>
      </li>
       <li class="selected03" style="display:none;">
        <div class="infor_l">
         <span>服务佣金</span>
		  <?php echo Html::input('text','agencycommissions',isset($datas['agencycommissiontype'])&&$datas['agencycommissiontype']==1?$datas['agencycommission']:'',['placeholder'=>'请输入费用','style'=>'width:55%;'])?>
    ...
    $(function(){
       ...
       /* $('.saveCls').click(function(){
        ...
        $('.publishCls').click(function(){
            ...
            saveCookie();
            save(true,id,category);
        });

        function save(tz,id,category){
            $.ajax({
                url:"<?php echo \yii\helpers\Url::toRoute(['/publish/editcollection','id'=>Yii::$app->request->get('id'),'category'=>Yii::$app->request->get('category')])?>",
                type:'post',
