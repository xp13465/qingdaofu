...
?>
  <?php  if(!empty($product) && !empty($uid) && !empty($certi)) { ?>
<?php if($product['progress_status'] == 1){$clock = 'clock';}else{$clock='';}?>
<?=wxHeaderWidget::widget(['title'=>'申请人详情','gohtml'=>'<div class="ck-header cp-header"><span class='.$clock.'></span></div>'])?>
...
<?php echo  \wx\widget\wxEvaluateWidget::widget(['pid'=>$pid]);?>
<footer>
<div class="apply apply1">
        <?php if($product['progress_status'] == 1){?>
            <div class="apply_sq">
                <a href="javascript:void(0)" class="apply_submit" data-id="<?php echo $product['id']?>" data-category="<?php echo $product['category']?>" data-pid="<?php echo $pid?>">同意申请</a>
            </div>
        <?php }else{ ?>
            <div class="apply_sq apply-sq01">
                <a href="javascript:void(0)" class="apply_submit">已同意</a>
            </div>
        <?php } ?>
    </div>
</footer>
...
<script type="text/javascript">
    $(function(){
        $(".apply_submit").click(function(){
             window.location = "<?php echo yii\helpers\Url::toRoute(['/protocol/index','id'=>$product['id'],'category'=>$product['category'],'uid'=>$pid])?>";
        })
    });
</script>
<?php } ?>
