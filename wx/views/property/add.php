...
<header>
    <div class="cm-header">
        ...
        <i>产调查询</i>
		<a href='<?php echo yii\helpers\Url::toRoute('/property/index')?>' >我的产调</a>
    </div>
</header>
...
<section>     
 <?php yii\widgets\ActiveForm::begin(['id'=>'propertyform'])?>
		    <ul class="infor" style="margin-top:20px;"> 
				<li>
		            <div class="infor_l">
		                <span>产调类型</span>
		                <?php 
						   echo Html::dropDownList('type', '2',
								//["1"=>"交易中心版","2"=>"电子版"],
								["2"=>"电子版"],
								[
									'class'=>'selects',
		    	...
		    	<li>
		            <div class="infor_l">
		                <span>产证区域</span>
		                <?php 
						   echo Html::dropDownList('area', '2705',
		        ...
		        <li class="infor_li">
		            <div class="infor_l">
		                <span>产证地址</span>                 
						<?= Html::input('text','address','',['placeholder'=>"请输入产证地址",'data-required'=>"true",])?>
		            </div>
		        </li> 
		        <li class="infor_li">
		            <div class="infor_l">
		                <span>电话号码</span>                 
						 <?= Html::input('text','phone',$defaultMobile,['placeholder'=>"请输入电话号码",'data-required'=>"true",])?>
		            </div>
		        </li>
				<li class="infor_li">
		            <div class="infor_l">
		                <span>产权人姓名</span>                 
						<?= Html::input('text','name','',['placeholder'=>"选填",'id'=>"name",'data-required'=>"false",])?>
		            </div>
		        </li> 
		    </ul>	
...
<p class='prompt_text'>产调目前暂停服务。</p>
<!--<p class='prompt_text'>产调目前只支持闵行区电子版，其他区暂停服务。</p>-->
...
-->
    ...
    <footer>
    <div class="zhen">      	
        <div  id="zl">总计<i style="color:red;font-size:18px;">￥25元</i></div>
        <button id="zr">点击支付</button>
    </div>
    </footer>
...
<script type="text/javascript">
    $(document).ready(function(){
		layer.msg("产调目前暂停服务。",{time:3000},function(){location.href='/site/index';})
		...
		$("#type").trigger("change")
		$('input[data-required="true"],select[data-required="true"]').on("change keyup",function(){
			if($(this).val()==''){
				$(this).css("color","red")
			}else{
				$(this).css("color","")
        ...
        $('#zr').click(function(){
			layer.msg("产调目前暂停服务。",{time:2000})
			return false;
			if($("#area").val() != 2705){
				layer.alert("产调目前只支持闵行区电子版，其他区暂停服务。",{title:'温馨提示'})
				return false;
			}
            ...
            var url = "<?php echo yii\helpers\Url::toRoute('/property/adddata')?>";
		...
		$("#area,#type").change(function(){
			...
			if(type=="1"){
				...
				if(area = '2705'){
					layer.alert("产调目前只支持闵行区电子版，其他区暂停服务。",{title:'温馨提示'},function(){$("#area").val("2705");layer.closeAll()})
				}
