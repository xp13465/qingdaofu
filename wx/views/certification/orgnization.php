    ...
    <div class="tupian">
        ...
        <?php }else{ ?>
		   ...
		   <div class="photo1" style="margin:-3% 25%;">
			   ...
			   <img src="<?php echo isset($v['file'])?Yii::$app->params['wx'].$v['file']:''?>" style="display:inline-block;" inputName="cardimg" inputNames="cardimgimg" limit="2" types='<?php echo $i++?>' data_bid="<?php echo $v['id'] ?>"/>
		    </div>
		   <?php } ?>
...
<section>
    ...
    <ul class="jb_x basic02">
        ...
        <li>
            ...
            <input type="text"  name="name" placeholder="请输入企业名称" style="font-size:14px;width:60%;" value="<?php echo isset($certi['name'])?$certi['name']:''?>"/>
        </li>
        <li>
            <span class="name">营业执照号</span>
            <input type="text" name="cardno" placeholder="请输入营业执照号" style="font-size:14px;width:65%;" value="<?php echo isset($certi['cardno'])?$certi['cardno']:''?>"/>
        </li>
        <li class="last_li">
    ...
    <ul class="jb_x basic01">
        ...
        <li>
            <span class="name">企业邮箱</span>
            <input type="text" name="email" placeholder="请输入企业邮箱" style="font-size:14px;width:60%;" value="<?php echo isset($certi['email'])?$certi['email']:''?>" />
        </li>
        <li>
            <span class="name">企业经营地址</span>
            <input type="text" name="address" placeholder="请输入企业经营地址" style="font-size:14px;width:60%;" value="<?php echo isset($certi['address'])?$certi['address']:''?>" />
        </li>
        <li>
            <span class="name">公司网站</span>
            <input type="text" name="enterprisewebsite" placeholder="请输入公司网站" style="font-size:14px;width:60%;" value="<?php echo isset($certi['enterprisewebsite'])?$certi['enterprisewebsite']:''?>" />
        </li>
				...
				if(data.code == '0000'&&data.result.fileid){
                    ...
                    $("input[name='" + inputName + "']").val((aa ? (aa + ",") : '')+"'"+data.result.url+"'").trigger("change"); 					
						...
						//if($('input[name=file]').attr('types')==1){
							var div = '<img src = "/images/close.gif" class="closebutton"/><img src="'+wx+data.result.url+'"  data_bid="'+data.result.fileid+'" style="display:inline-block;"  inputName="cardimg" inputNames="cardimgimg" limit="2" types="1"/>';	
